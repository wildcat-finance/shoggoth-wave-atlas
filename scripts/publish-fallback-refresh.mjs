...
if (autoMerge && !process.env.PR_TOKEN) {
  throw new Error("AUTO_MERGE requires PR_TOKEN; refusing a partly automatic refresh.");
}
...
const refreshOid = commit.createCommitOnBranch.commit.oid;
...
if (autoMerge) {
  // Refuse to fold a snapshot prepared against an old base into a newer main.
  // The next scheduled run will regenerate from that new base instead.
  const latestBase = await rest("GET", `/git/ref/heads/${BASE_BRANCH}`);
  if (latestBase.body?.object?.sha !== baseOid) {
    throw new Error(
      `${BASE_BRANCH} advanced while the refresh was publishing; leaving pull request ` +
        `#${pull.number} open for the next run.`,
    );
  }
